...
export default function Dashboard() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // Keep previous values when gateway is unavailable.
    }
  }, []);
  ...
  return (
    <main className="app-shell">
      <h1 className="mb-8 text-[54px] font-bold text-slate-100">MetaCrawler Dashboard</h1>
      ...
      <section className="panel mt-6 p-6">
        <h2 className="text-[40px] font-semibold text-slate-200">Recent Jobs</h2>

        <div className="mt-4 overflow-x-auto">
          <table className="min-w-full text-left text-[26px]">
          ...
          {!jobs.length && <p className="px-2 py-5 text-[30px] text-slate-600">No jobs yet.</p>}
        </div>
      </section>
    </main>
  );
